...
pub fn assemble(source: &str) -> Result<Vec<MachineCode>, AssemblyPipelineError> {
    ...
    let table =
        get_symbol_table(&origins).map_err(|(_, e)| AssemblyPipelineError::AssemblyError(e))?;
    let codes =
        assemble_origins(&origins, &table).map_err(|e| AssemblyPipelineError::AssemblyError(e))?;
    Ok(codes)
}

pub fn assemble_origins(
    origins: &[Origin],
    symbol_table: &SymbolTable,
) -> Result<Vec<MachineCode>, Vec<AssemblyError>> {
...
pub fn assemble_origin(
    origin: &Origin,
    symbol_table: &SymbolTable,
) -> Result<MachineCode, Vec<AssemblyError>> {
    ...
    for statement in &origin.statements {
        ...
        match assemble_statement(&statement.kind, pc, symbol_table) {
            Err(e) => errors.push(e),
            Ok(assembled_statement) if errors.len() == 0 => {
                machine_code.extend(assembled_statement)
            }
...
pub fn assemble_statement(
    ...
    let words = match statement_kind {
        StatementKind::AddAnd(kind, r0, r1, immediate_or_r2) => {
            ...
            vec![
                instruction
                    | (r0.get_truncated_u16() << 9)
                    | r1.get_truncated_u16() << 6
                    | termination,
            ]
        }
        ...
        StatementKind::Imm9MemInstruction(kind, r0, offset) => {
            ...
            vec![
                instruction
                    | r0.get_truncated_u16() << 9
                    | symbol_table.get_or_give_offset::<9>(pc, offset)?,
            ]
        }
        StatementKind::MemRelative(kind, dr, base, offset) => {
            ...
            vec![
                instruction
                    | dr.get_truncated_u16() << 9
                    | base.get_truncated_u16() << 6
                    | offset.get_truncated_u16(),
            ]
        }
        StatementKind::Not(r0, r1) => vec![
            lc3_constants::NOT
                | r0.get_truncated_u16() << 9
                | r1.get_truncated_u16() << 6
                | (1 << 6) - 1,
...
fn get_statement_size(statement_kind: &StatementKind) -> u16 {
    match statement_kind {
        ...
        StatementKind::Stringz(s) => (s.len() + 1)
            .try_into()
            .expect("the string is not longer than u16::MAX-1"),
        _ => 1,
    }
}
...
fn get_symbol_table(origins: &[Origin]) -> Result<SymbolTable, (SymbolTable, Vec<AssemblyError>)> {
    ...
    for origin in origins {
        ...
        for statement in &origin.statements {
            if let Err(ae) = pc
                .increment(last_instuction_size)
                .map_err(|_| AssemblyError::LabelOutOfRange(pc.get() + last_instuction_size))
            {
                errors.push(ae);
            }
...
impl<'a> SymbolTable {
    ...
    pub fn get_offset<const BITS: u32>(&self, pc: u16, symbol: &str) -> Result<u16, AssemblyError> {
        ...
        let n_bit_number = NBitInt::<BITS, true>::new(signed_offset).map_err(|e| {
            AssemblyError::OffsetOutOfRange(symbol.to_string(), e.attempted_num, e.bits as u8)
        })?;
        Ok(n_bit_number.get_truncated_u16())
    }

    pub fn get_or_give_offset<const BITS: u32>(
        &self,
        pc: u16,
        offset_or_symbol: &Either<NBitInt<BITS, true>, String>,
    ) -> Result<u16, AssemblyError> {
        let n_bit_offset = match offset_or_symbol {
            Either::A(offset) => *offset,
            Either::B(symbol) => {
                let signed_offset = self.get(symbol)? as i32 - pc as i32;
                NBitInt::<BITS, true>::new(signed_offset).map_err(|e| {
...
impl fmt::Display for AssemblyError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::DoubleDefinedSymbol(sym) => {
                write!(f, "the label \"{}\" has been defined more than once", sym)
            }
...
mod tests {
    ...
    fn symbol_table_one_origin_and_all_statements_have_label() {
        ...
        let origins = parse_lexemes(&lexemes).unwrap();
        let table = get_symbol_table(&origins).unwrap();
        let mut labels_and_addresses: Vec<_> = table
            .labels()
            .iter()
            .map(|&l| (l, table.get(l).unwrap()))
            .collect();
        labels_and_addresses.sort_by(|a, b| a.0.cmp(b.0));
        assert_eq!(
            labels_and_addresses,
            vec![("l1", 300), ("l2", 300), ("l3", 301), ("l4", 302),]
        );
    }
    ...
    fn symbol_table_assigns_correctly_with_stringz() {
        ...
        assert_eq!(
            labels_and_addresses,
            vec![("l1", 0x3000), ("l2", 0x3000), ("l3", 0x3005),]
        );
    }
    ...
    fn assembles_all_branch_instruction_possibilities() {
        assert_eq!(
            ...
            vec![MachineCode {
                start_address: Address::new(0x3000).unwrap(),
                code: vec![
                    0x0FFF, 0x09FF, 0x05FF, 0x0DFF, 0x03FF, 0x0BFF, 0x07FF, 0x0FFF,
                ]
            }]
        )
    }
    ...
    fn assembles_fill() {
        assert_eq!(
            ...
            vec![MachineCode {
                start_address: Address::new(0x3000).unwrap(),
                code: vec![0xFFFF, 16, 2_u16.pow(15) + 1, 0x3000, 0x3001,]
            }]
        )
    }
    ...
    fn assembles_blkw() {
        assert_eq!(
            ...
            vec![MachineCode {
                start_address: Address::new(0x3000).unwrap(),
                code: vec![0xFFFF, 0, 0, 0, 0, 0x1234, 0, 0, 0, 0, 0, 0x4321,]
            }]
        )
    }
